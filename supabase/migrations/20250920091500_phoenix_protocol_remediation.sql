-- =====================================================================================
-- PHOENIX PROTOCOL: Live Order Pipeline Critical Remediation
...
-- Payment Intents Table (Missing entirely from current schema)
CREATE TABLE IF NOT EXISTS public.payment_intents (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    payment_intent_id TEXT UNIQUE NOT NULL, -- Provider's payment intent ID
    ...
    provider TEXT NOT NULL, -- 'stripe', 'esewa', 'khalti', 'mock'
    client_secret TEXT, -- For frontend payment processing
    provider_response JSONB, -- Full provider response data
    ...
    CONSTRAINT unique_payment_intent_provider UNIQUE (provider, payment_intent_id)
);

CREATE INDEX idx_payment_intents_customer ON payment_intents (customer_id);
CREATE INDEX idx_payment_intents_cart ON payment_intents (cart_id);
CREATE INDEX idx_payment_intents_status ON payment_intents (status);
CREATE INDEX idx_payment_intents_expires ON payment_intents (expires_at) WHERE status = 'pending';
...
BEGIN
    ...
    LOOP
        ...
        RETURNING variant_id INTO v_item.variant_id;
...
BEGIN
    ...
    v_customer_id := v_payment_intent.customer_id;
...
-- Policy: Users can only view their own payment intents
...
-- Schedule this to run periodically (via cron job or external scheduler)
-- Example: SELECT cleanup_expired_reservations();
...
-- Summary of fixes:
-- 1. ✅ Created missing payment_intents table

-- =====================================================================
-- CRITICAL HOTFIX: Booking Engine Column Name Correction
-- =====================================================================
-- This migration fixes the fatal defect identified in the CTO audit where
-- get_available_slots was querying non-existent columns in stylist_schedules
-- =====================================================================
...
RETURNS TABLE (
    ...
    slot_display text,
    status text,  -- Unified status field (no redundant is_available)
    price_cents integer
)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- CRITICAL FIX: Use correct column names (stylist_user_id, is_active)
    SELECT * INTO v_schedule
    FROM public.stylist_schedules
    WHERE stylist_user_id = p_stylist_id  -- FIXED: was stylist_id
      AND day_of_week = v_day_of_week
      AND is_active = true;  -- FIXED: was is_available
...
-- Add helpful comment
COMMENT ON FUNCTION public.get_available_slots IS 'HOTFIXED: Booking slot availability function. Fixed column references to use stylist_user_id and is_active. Returns slots with status (available/booked/reserved) for a stylist-service combination on a given date.';
...
DO $$
BEGIN
    RAISE NOTICE 'HOTFIX APPLIED: get_available_slots now uses correct column names (stylist_user_id, is_active)';
END $$;

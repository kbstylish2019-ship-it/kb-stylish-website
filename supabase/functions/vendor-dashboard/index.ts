...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Call SECURITY INVOKER RPC with userClient (JWT propagation)
    // Optional: Parse vendor_id from query params for admin override
    const url = new URL(req.url);
    const vendorId = url.searchParams.get('vendor_id');
    
    const rpcParams = vendorId ? { v_id: vendorId } : {};
    ...
    if (error) {
      ...
      // Check for permission denied (RLS violation)
      if (error.code === '42501') {
        return errorResponse(
          'Access denied: Cannot access other vendor data',
          'FORBIDDEN',
          403,
          cors
        );

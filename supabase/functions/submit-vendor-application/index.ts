 ...
 * Version: 1.0 (Growth Engine - Phase 2)
 * 
 * Purpose: Accept vendor application submissions from authenticated users
 * Security: JWT authentication required, calls SECURITY DEFINER RPC
 * Pattern: Follows proven dual-client pattern from cart-manager v3.0
 * 
 * Flow:
 * 1. Verify user authentication
 * 2. Validate request body
 * 3. Call submit_vendor_application_secure RPC
...
interface VendorApplicationRequest {
  ...
  phone: string;
  website?: string;
  payout_method: 'bank' | 'esewa' | 'khalti';
  bank_name?: string;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!user) {
      ...
      return errorResponse(
        'Unauthorized. Please log in to submit an application.',
        'AUTH_REQUIRED',
        401,
        corsHeaders
      );
    ...
    // ========================================================================
    let applicationData: VendorApplicationRequest;
    ...
    if (!applicationData.business_name || !applicationData.business_type || 
        !applicationData.email || !applicationData.phone || !applicationData.payout_method) {
      return errorResponse(
        'Missing required fields: business_name, business_type, email, phone, payout_method',
        'MISSING_FIELDS',
        400,
        corsHeaders
      );
    ...
    const { data: rpcResult, error: rpcError } = await serviceClient
      .rpc('submit_vendor_application_secure', {
        p_user_id: user.id,  // ✅ NEW: Pass validated user ID from JWT
        p_application_data: {

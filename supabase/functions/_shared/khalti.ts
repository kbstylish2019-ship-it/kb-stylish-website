...
export interface KhaltiPaymentRequest {
  amount: number; // Amount in NPR (will be converted to paisa internally)
...
/**
 * Khalti payment initiation response
 */
...
/**
 * Khalti transaction verification response
 */
...
export async function verifyKhaltiTransaction(
  ...
  try {
    ...
    // Status validation
    const validStatuses = ['Completed', 'Pending', 'User canceled', 'Expired', 'Refunded'];
 ...
 */
export function compareKhaltiAmount(khaltiAmountPaisa: number, expectedAmountNPR: number): boolean {

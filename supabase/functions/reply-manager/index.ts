 ...
 */ async function handleSubmitReply(request, userClient, serviceClient, authenticatedUser, cors) {
  ...
  // Check RPC response
  if (!data?.success) {
    return errorResponse(data?.error || 'Failed to submit reply', data?.error_code || 'SUBMISSION_FAILED', 400, cors);
  }
  const response = {
    success: true,
    reply: {
 ...
 */ async function handleUpdateReply(request, serviceClient, authenticatedUser, cors) {
  ...
  const { data, error } = await serviceClient.from('review_replies').update({
    comment: sanitizedComment,
    is_edited: true,
    updated_at: new Date().toISOString()
  }).eq('id', reply_id).eq('user_id', authenticatedUser.id) // Ensure ownership
  .eq('reply_type', 'vendor').is('deleted_at', null).select().single();
  if (error) {
 ...
 */ async function handleDeleteReply(request, serviceClient, authenticatedUser, cors) {
  ...
  const { error } = await serviceClient.from('review_replies').update({
    deleted_at: new Date().toISOString()
  }).eq('id', reply_id).eq('user_id', authenticatedUser.id).eq('reply_type', 'vendor').is('deleted_at', null);
  if (error) {
 ...
 */ async function handleFetchReplies(request, serviceClient) {
  ...
  const { data: replies, error } = await serviceClient.from('review_replies').select(`
      id,
      review_id,
      comment,
      is_edited,
    ...
    `).eq('reply_type', 'vendor').eq('reviews.product_id', product_id).eq('is_visible', true).eq('is_approved', true).is('deleted_at', null).order('created_at', {
    ascending: false
  });
  if (error) {

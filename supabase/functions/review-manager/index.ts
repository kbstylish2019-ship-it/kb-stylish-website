 ...
 */ async function handleFetchReviews(request, serviceClient, authenticatedUser, cors) {
  ...
  const selectColumns = `
    ...
    `;
  // Build query
  let query = serviceClient.from('reviews').select(selectColumns);
  // Apply filters
  ...
  if (filters.sortBy === 'helpful') {
    query = query.order('helpful_votes', {
      ascending: false
    });
  }
  ...
  if (error) {
    ...
    return errorResponse(`Failed to fetch reviews: ${error.message || 'Unknown error'}`, 'QUERY_ERROR', 400, cors);
  }
  // Batch fetch user votes if authenticated
  let userVotesMap = new Map();
  if (authenticatedUser && reviews && reviews.length > 0) {
    const reviewIds = reviews.map((r)=>r.id);
    const { data: userVotes } = await serviceClient.from('review_votes').select('review_id, vote_type').eq('user_id', authenticatedUser.id).in('review_id', reviewIds);
    if (userVotes) {
      userVotesMap = new Map(userVotes.map((v)=>[
          v.review_id,
          v.vote_type
        ]));
  ...
  const processedReviews = (reviews || []).map((review)=>{
    ...
    const totalHelpful = review.review_vote_shards?.reduce((sum, shard)=>sum + (shard.helpful_count || 0), 0) || 0;
    const totalUnhelpful = review.review_vote_shards?.reduce((sum, shard)=>sum + (shard.unhelpful_count || 0), 0) || 0;
    // Vendor reply normalization - pick only vendor reply if array
    let vendorReply = null;
    const vr = review.vendor_reply ?? null;
    if (Array.isArray(vr)) {
      vendorReply = vr.find((r)=>r.reply_type === 'vendor' && r.is_visible && r.is_approved && r.deleted_at === null) || null;
    ...
    return {
      ...
      vendor_reply: vendorReply ? {
        id: vendorReply.id,
        comment: vendorReply.comment,
        created_at: vendorReply.created_at
      } : null,
  ...
  let stats = null;
  if (filters.productId) {
    const { data: product } = await serviceClient.from('products').select('average_rating, review_count, rating_distribution').eq('id', filters.productId).single();
    if (product) {
      stats = {
  ...
  const nextCursor = processedReviews.length === limit ? processedReviews[processedReviews.length - 1].created_at : undefined;
 ...
 */ async function handleSubmitReview(request, userClient, authenticatedUser, cors) {
  ...
  // Check RPC response
  if (!data.success) {
    return errorResponse(data.error || 'Failed to submit review', data.error_code || 'SUBMISSION_FAILED', 400, cors);
  }
  return new Response(JSON.stringify({
    success: true,
    review: {

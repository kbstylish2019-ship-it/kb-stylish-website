 ...
 */ Deno.serve(async (req)=>{
  ...
  try {
    ...
    const { data: jobs, error: fetchError } = await serviceClient.from('job_queue').select('id, payload, attempts').eq('job_type', 'update_product_rating').eq('status', 'pending').is('locked_until', null).order('priority', {
      ascending: true
    }).order('created_at', {
      ascending: true
    }).limit(10);
    ...
    for (const job of jobs){
      try {
        ...
        const { error: lockError } = await serviceClient.from('job_queue').update({
          status: 'processing',
          locked_until: lockUntil,
          attempts: (job.attempts || 0) + 1
        }).eq('id', job.id).eq('status', 'pending'); // Only lock if still pending
        if (lockError) {
        ...
        const { data: result, error: rpcError } = await serviceClient.rpc('update_product_rating_stats', {
        ...
        if (rpcError || !result?.success) {
          ...
          if (shouldRetry) {
            ...
            await serviceClient.from('job_queue').update({
              status: 'pending',
              locked_until: null,
              last_error: rpcError?.message || result?.error || 'RPC failed'
            }).eq('id', job.id);
            ...
            await serviceClient.from('job_queue').update({
              status: 'failed',
              last_error: rpcError?.message || result?.error || 'Max attempts reached',

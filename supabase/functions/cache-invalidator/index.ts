 ...
 * 
 * Listens for PostgreSQL NOTIFY events and invalidates Upstash Redis cache entries
 ...
 */ async function processProductChange(payload) {
  ...
  try {
    ...
    if (url.pathname === '/invalidate' && req.method === 'POST') {
      ...
      if (!keys || !Array.isArray(keys)) {
        return new Response('Invalid request: keys array required', {
          status: 400
        });
      }

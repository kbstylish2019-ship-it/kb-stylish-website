...
export default async function AdminCategoriesPage() {
  ...
  if (!categories) {
    return (
      <DashboardLayout title="Categories" sidebar={<AdminSidebar />}>
        <div className="rounded-2xl border border-red-500/20 bg-red-500/10 p-6">
          <h2 className="text-lg font-semibold text-red-500">Failed to Load Categories</h2>
          <p className="mt-2 text-sm text-red-400">
            Unable to fetch categories. Please refresh the page or try again later.
          </p>
        </div>
      </DashboardLayout>
    );
